...
import { Plus, Eye, Pencil, Search, Trash, ChevronRight, Play } from 'lucide-react'
...
export default function CursoClient() {
  ...
  const [bancos, setBancos] = React.useState<BanqueosSimulacrosGET>([]);
  ...
  React.useEffect(() => {
    setLoading(true);
    fetcher<BanqueosSimulacrosGET>(`/banqueos/simulacros?search=${search}&areas=${selectedAreas.join(',')}&temas=${selectedTemas.join(',')}&capitulos=${selectedCapitulos.join(',')}`).then(setBancos).finally(() => setLoading(false));
  ...
  return (
    <div className="container mx-auto  space-y-4">
      <h1 className="text-3xl">Simulacros Nexus</h1>
      <p className="text-muted-foreground">Encuentra simulacros creados para la preparación de examenes de admisión universitaria</p>
      <Card>
        <CardContent>
        ...
        <CardContent>
          {
            bancos.length === 0 && (
              <Empty>
                ...
                <EmptyHeader>
                  <EmptyTitle>No se encontraron simulacros</EmptyTitle>
                  <EmptyDescription>
                    Intenta ajustar tus filtros de búsqueda para encontrar simulacros que se ajusten a tus necesidades.
                  </EmptyDescription>
                </EmptyHeader>
              </Empty>
            )
